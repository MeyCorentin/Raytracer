...
Math::Point3D Math::VecToPoint(const Math::Vector3D &v)
{
    return Math::Point3D(v.x_coords, v.y_coords, v.z_coords);
}
Math::Vector3D Math::PointToVec(const Math::Point3D &v)
{
    return Math::Vector3D(v.x_coords, v.y_coords, v.z_coords);
}
