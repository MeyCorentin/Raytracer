...
int main() {
    ...
    // Render
    Raytracer::Sphere sphere(new Math::Point3D(0, 0, -1), 0.5);
    ...
    for (int j = image_height - 1; j >= 0; --j) {
        for (int i = 0; i < image_width; ++i) {
            ...
            Ray *r = cam.ray(u, v);
            Math::Point3D pixel_color = sphere.hits(*r);
            ppm_output(std::cout, Math::Vector3D(pixel_color.x_coords, pixel_color.y_coords, pixel_color.z_coords));
        }
    }
